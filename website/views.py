    ...
    if session["ID"] == "admin":
        if request.method == 'POST':
            ...
            if 'reveal' in request.form:
                ...
                engine.log(f"Passage à la page suivante : {pages[gameState['iterator']]['url']} (jeu {engine.current_stage[0]}, manche {engine.current_stage[1]})")
                ...
                gameState['done'] = 0
                gameState['iterator'] -= 1
                engine.log(f"Retour à la page précedente : {pages[gameState['iterator']]['url']} (jeu {engine.current_stage[0]}, manche {engine.current_stage[1]})")
                save_data()
                refresh_all_pages()
    ...
    if request.method == 'POST':
        ...
        if pages[gameState['iterator']]['url'] == "Jeu2-choix.html":
            ...
            if request.form["boutton"] == "validate num":
                ...
                player["done"] = True
                gameState['done'] += 1
                engine.log(f"{player.name} a choisis le nombre {player['choix']}.")
                flash(f"Vous avez choisis le nombre {player['choix']}.", category="success")
                if gameState['done'] < 5:
                    update_waiting_count(gameState["done"], 5)
                else:
                    game2_logic()
        ...
        if request.form["boutton"] == "Jeu3-choix":
            ...
            player["done"] = True
            gameState["done"] += 1
            engine.log(f"{player.name} a versé {montant} Pièces dans le pot commun")
            flash(Markup(f"Vous avez versé {montant} {icons['coin']} dans le pot commun"), category="success")
            if gameState['done'] < 5:
                update_waiting_count(gameState["done"], 5)
            else:
                game3_logic()
        ...
        if pages[gameState['iterator']]['url'] == "Jeu4-choix.html":
            ...
            if request.form["boutton"] == "Jeu4-choix":
                ...
                if prize[player["choix"]] == "star":
                    ...
                    engine.log(f"{player.name} a choisis le prix : {prize[player['choix']]} Pièces")
                    flash(Markup(f"Vous avez choisis le prix : {prize[player['choix']]} {icons['coin']}"), category="success")
                if gameState['done'] < 5:
                    update_waiting_count(gameState["done"], 5)
                else:
                    game4_logic()
        ...
        if request.form["boutton"] == "terminer":
            ...
            if gameState['done'] == 5:
                ...
                engine.log(f"Le nombre d'etoiles pour chaque joueur est respectivement : {stars}")
                if stars.count(max(stars)) == 1:
                    gameState['starMaster'] = players[winnerID]
                    gameState['starMaster']['flouze'] += games_config['game5']['prize'] + games_config['game5']['bonus'] * gameState['masterPrizeBonus']
                    gameState['otherPlayers'].remove(players[winnerID])
                    for p in players:
                        p['message'] = Markup(f"{gameState['starMaster']['name']} a le plus d'étoiles et est ainsi en possesion de la somme de {games_config['game5']['prize'] + games_config['game5']['bonus'] * gameState['masterPrizeBonus']} {icons['coin']} pour le 5ème jeu")
                    gameState['starMaster']['message'] = Markup(f"Vous avez le plus d'étoiles et êtes ainsi en possesion de la somme de {games_config['game5']['prize'] + games_config['game5']['bonus'] * gameState['masterPrizeBonus']} {icons['coin']} pour le 5ème jeu")
                    engine.log(f"{gameState['starMaster']['name']} a le plus d'étoileset remporte ainsi la somme de {games_config['game5']['prize']} pièces pour le cinquième jeu.")
                else:
                    engine.log("Dû à une égalité en terme d'étoiles, personne ne remporte le gros lot et le cinquième jeu est annulé")
                    for p in players:
        ...
        if pages[gameState['iterator']]['url'] == "Jeu 5":
            if request.form["boutton"] == 'quiz':
                engine.log(f'{player.name} a donner la réponse "{request.form.get("réponse")}" au quiz')
            ...
            if request.form["boutton"] == 'proposition':
                ...
                if total > gameState['starMaster']['flouze']:
                    ...
                    return render_template("Jeu5-proposition.html", theme_color=theme_colors[pages[gameState['iterator']]['background']][0], user=player, otherPlayers=gameState['otherPlayers'], background=pages[gameState['iterator']]['background'])
                for player in gameState['otherPlayers']:
                    montant = int(request.form.get(p['name']))
                    player["proposition"] = montant
                    player["message"] = Markup(f"{player.name} vous fait une proposition de {montant} {icons['coin']}")
                gameState['iterator'] += 1
                refresh_all_pages()
                ...
                if gameState['done'] < 4:
                    ...
                    if sum(p['choix'] for p in gameState['otherPlayers']) >= 3:
                        ...
                        for p in gameState['otherPlayers']:
                            ...
                            p['message'] = Markup(f"La proposition à été acceptée par la majorité des joueurs.\n Vous avez recu {p['proposition']} {icons['coin']}")
                        game5_done()
                    else:
                        gameState['remaining_trials'] -= 1
                        if gameState['remaining_trials'] == 0:
                            gameState['starMaster']['flouze'] -= games_config['game5']['prize'] + games_config['game5']['bonus'] * gameState['masterPrizeBonus']
                            gameState['starMaster']['message'] = Markup(f"Votre dernière proposition a été refusée par la majorité. Les {games_config['game5']['prize'] + games_config['game5']['bonus'] * gameState['masterPrizeBonus']} {icons['coin']} vous sont donc retirés")
                            for p in gameState['otherPlayers']:
                                p['message'] = Markup(f"Auccun accord à été trouvé apprès ces 3 essais donc {gameState['starMaster']['name']} ne remporte pas les {games_config['game5']['prize'] + games_config['game5']['bonus'] * gameState['masterPrizeBonus']} {icons['coin']}")
                            game5_done()
                        else:
